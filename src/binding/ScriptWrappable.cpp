...
// ScriptValue = Escargot::ValueRef*
// ScriptArrayObject = Escargot::ArrayObjectRef*
template <typename U>
    ...
    return Evaluator::execute(
               ...
               [](ExecutionStateRef* state,
                  const std::vector<U>* vector) -> ValueRef* {
                   ...
                   Escargot::ValueVectorRef* valueVector =
                       Escargot::ValueVectorRef::create(arrayLength);
                   for (size_t i = 0; i < arrayLength; ++i) {
                       valueVector->set(i, ValueRef::create(vector->at(i)));
                   }

                   // create(ExecutionStateRef* state)
                   // create(ExecutionStateRef* state, const uint64_t size)
                   // create(ExecutionStateRef* state, ValueVectorRef* source)
                   return Escargot::ArrayObjectRef::create(state, valueVector);
               },
               &vector)
