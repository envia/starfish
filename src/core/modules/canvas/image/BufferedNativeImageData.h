...
public:
    ...
    // free-list link in a non-debug collector), and the disclaim proc would then
    // dereference that garbage as a vtable and crash. The destructor above has
    // already disposed the decoded buffer, so instead of freeing, clear the
