...
public:
              ...
              const GLenum type)
    {
        ...
        // NativeImageData (canvas/image) is always 4 bytes per pixel (BGRA/RGBA).
        // ArrayBufferView source matches the requested format byte count.
        ...
        // GL_RGB requires 3 bytes per pixel in the upload buffer; GL_RGBA needs 4.
    ...
    std::function<void(const TexImageHelper*)> updateImage)
{
    ...
    // Canvas/image data is always tightly packed with no row padding. Force
    // GL_UNPACK_ALIGNMENT=1 so OpenGL ES reads rows at the correct byte
    // offsets regardless of row width (e.g. RGB with non-multiple-of-4 width).
