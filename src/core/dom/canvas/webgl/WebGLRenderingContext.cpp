...
void WebGLRenderingContext::flushForReadback()
{
    ...
    // The application controls the pixel pack state, but this readback
    // must write rows of exactly `stride` bytes into client memory.
    // Override the pack parameters and the pack buffer binding for the
    // internal readPixels() and restore them after.
    ...
    for (size_t top = 0; top < height / 2; ++top) {
        ...
        for (size_t col = 0; col < width; ++col) {
            uint8_t* pTop = buffer + top * stride + col * 4;
            uint8_t* pBot = buffer + bot * stride + col * 4;
            memcpy(tmp, pTop, 4);
            memcpy(pTop, pBot, 4);
            memcpy(pBot, tmp, 4);
        }
