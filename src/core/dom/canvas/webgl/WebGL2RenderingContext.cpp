                                              ...
                                              GLuint length)
{
    ...
    size_t elementSize =
        (dstBuffer->isDataViewObject() || dstLength == 0)
            ? 1
                                        ...
                                        unsigned long long dstOffset)
{
    ...
    if (width == 0 || height == 0) {
        // Still let GL validate the framebuffer and format/type combination.
        gl()->readPixels(x, y, width, height, format, type, nullptr);
        return;
    }
