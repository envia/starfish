...
void WebGLProgram::bindAttribLocation(GLuint index, const std::string& name)
{
    m_attribLocationBindings[name] = index;
}
...
bool WebGLProgram::hasAliasedAttribLocations() const
{
    ...
    gl->getProgramiv(programObj, GL_ACTIVE_ATTRIBUTE_MAX_LENGTH, &maxNameLength);

    // Collect locations of active attributes that were bound via bindAttribLocation
    ...
    for (GLint i = 0; i < numActiveAttribs; i++) {
        ...
        gl->getActiveAttrib(programObj, i, maxNameLength, &length, &size, &type, nameBuffer.data());
        ...
        if (it != m_attribLocationBindings.end()) {
            ...
            // Check if this location is already used by another active attribute
