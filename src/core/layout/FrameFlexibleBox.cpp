    ...
    , m_crossAxisGap(0)
{
    ...
    // column-gap separates items along the inline axis, row-gap along the
    // block axis. Map them onto the container's main/cross axes.
    if (m_isMainAxisInInlineAxis) {
        m_mainAxisGap = columnGap;
        m_crossAxisGap = rowGap;
    } else {
        m_mainAxisGap = rowGap;
        m_crossAxisGap = columnGap;
    }
...
void FlexFormattingContext::computeMainSize()
{
    ...
    while (iter != orderedFlexItems.end()) {
        ...
        // A gap precedes every item except the first one on the line.
        if (flexItems.size()) {
            requiredMainSizeForItemInFlexLine += m_mainAxisGap;
        }
        ...
        if (m_isSingleLine || (lineMainSize == 0) || canPlaceToFlexLine) {
            if (flexItems.size() > 0) {
                lineMainSize += m_mainAxisGap;
                flexLine.m_sumOfMainAxisGapInComputeMainSize += m_mainAxisGap;
            }
...
void FlexFormattingContext::applyFlexFactor()
{
    ...
    for (size_t i = 0; i < lines; i++) {
        ...
        initialFreeSpace -= flexLine.m_sumOfMainAxisGapInComputeMainSize;
        ...
        while (!isAllFrozen) {
            ...
            if (!isAllFrozen) {
                ...
                remainingFreeSpace -=
                    flexLine.m_sumOfMainAxisGapInComputeMainSize;
            }
        }
    }
}
...
void FlexFormattingContext::resolveMainMargin()
{
    ...
    for (size_t i = 0; i < lines; i++) {
        ...
        bool canResolve =
            m_availableMainSize >
            sumOfMainSize + flexLine.m_sumOfMainAxisGapInComputeMainSize;
        ...
        if (m_isMainAxisInInlineAxis) {
            if (m_isLtrDirection) {
                ...
                for (size_t j = 0; j < flexItems.size(); j++) {
                    ...
                    x += flexItem->outerWidth() + separator + m_mainAxisGap;
                }
            } else {
            ...
            if (m_isTtbDirection) {
                ...
                for (size_t j = 0; j < flexItems.size(); j++) {
                    ...
                    y += flexItem->outerHeight() + separator + m_mainAxisGap;
                }
            } else {
...
void FlexFormattingContext::computeCrossSize()
{
    ...
    // Gaps between flex lines count toward the cross size occupied by the
    // content, so the container grows to contain them.
    sumOfCrossSize += m_crossAxisGap * (lines - 1);
    ...
    if (m_isMainAxisInInlineAxis) {
        if (m_isTtbDirection) {
            ...
            for (size_t i = 0; i < lines; i++) {
                ...
                y += flexLine.m_lineHeight + separator + m_crossAxisGap;
            }
        } else {
        ...
        if (m_isLtrDirection) {
            ...
            for (size_t i = 0; i < lines; i++) {
                ...
                x += flexLine.m_lineHeight + separator + m_crossAxisGap;
            }
        } else {
