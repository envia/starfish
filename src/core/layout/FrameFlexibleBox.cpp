...
LayoutUnit FlexFormattingContext::automaticMinimumMainSize(FrameBox* flexItem)
{
    ...
    // cross size must be used here; only an auto-and-indefinite cross size falls
    // back to fit-content. A stretch-aligned item in a container with a definite
    // cross size has a definite (stretched) cross size, so measure at that
    // width/height. Otherwise the item (e.g. a nested flex container) would be
    // sized to its fit-content cross size, wrapping its text and overstating the
    // main size.
    ...
    if (m_availableCrossSize != intMaxForLayoutUnit &&
        ...
        if (m_isMainAxisInInlineAxis) {
            crossSize = m_availableCrossSize -
                        (contentBox ? flexItem->mbpHeight()
                                    : flexItem->marginHeight());
        } else {
            crossSize = m_availableCrossSize -
                        (contentBox ? flexItem->mbpWidth()
                                    : flexItem->marginWidth());
        }
        if (crossSize < 0) {
            crossSize = 0;
        }
        // The cross-axis size is auto here (required for stretch), so it is safe
        // to restore it to auto afterwards.
        if (m_isMainAxisInInlineAxis) {
...
bool FlexFormattingContext::isStretchedAlongCrossAxis(FrameBox* flexItem)
{
    ...
    if (m_isMainAxisInInlineAxis) {
        return flexItem->style()->height().isAuto() &&
