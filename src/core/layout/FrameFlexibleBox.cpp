...
void FlexFormattingContext::applyAlignContent()
{
    ...
    // Inter-line gaps occupy cross space just like the lines themselves, so
    // they must be subtracted from the free space before align-content
    // distributes the remainder (mirrors applyJustifyContent()).
    if (lines > 0) {
        sumOfCrossSize += m_crossGap * (lines - 1);
    }
