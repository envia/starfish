...
> **Tags accepted but exposed as generic `HTMLElement` (no element-specific DOM API):** `center`, `i`, `s`, `dfn`, `b`, `u`, `mark`, `strong`, `cite`, `em`, `var`, `address`, `article`, `aside`, `footer`, `header`, `hgroup`, `main`, `nav`, `section`, `summary`, `code`, `dt`, `dd`. The `summary` element uses `HTMLElement` as specified, with native disclosure activation. Any other custom or HTML5 tag the parser doesn't recognize (`figure`, `time`, `picture`, `kbd`, `small`, `wbr`, `ruby`, …) becomes `HTMLUnknownElement` — they parse and lay out as inline boxes but expose no element-specific DOM members.

> **Disclosure scope:** `HTMLDetailsElement.open` and `name` reflect attributes.
> The first summary supports click, Enter and Space activation; interactive
> descendants retain their own actions. Closed content is hidden in an internal
> shadow tree. Find-in-page / fragment-navigation auto-expansion,
> `::details-content`, and content-visibility-based layout APIs are not supported.
> Disclosure markers currently use horizontal left-to-right glyphs.
...
| | misc | **Unsupported in LWE** (IDL `[Unimplemented]` — assigning to the handler succeeds, but the event never fires) | `onauxclick`, `oncancel`, `onclose`, `oncontextmenu`, `oncuechange`, `ondblclick`, `ondrag`/`ondragend`/`ondragenter`/`ondragexit`/`ondragleave`/`ondragover`/`ondragstart`/`ondrop`, `oninvalid`, `onloadend`, `onwheel`, `onreset`, `onselect`, `onshow`, `onpointerover`/`onpointerenter`/`onpointercancel`/`onpointerout`/`onpointerleave`/`ongotpointercapture`/`onlostpointercapture`/`onpointerrawupdate`. **Drag-and-drop and the wheel event are unsupported on LWE.** |
| [ToggleEvent](https://html.spec.whatwg.org/multipage/interaction.html#the-toggleevent-interface) | interface | ToggleEvent | Window-exposed constructor; readonly `oldState`, `newState`, and shadow-retargeted `source`. Details fires trusted, non-bubbling, non-cancelable toggle events. |
| | handler | ontoggle | Supported on GlobalEventHandlers; includes the HTML `ontoggle` content attribute. |
...
| [Display](https://www.w3.org/TR/CSS2/visuren.html#display-prop) | display | inline &#124; block &#124; inline-block &#124; table &#124; inline-table &#124; table-row-group &#124; table-header-group &#124; table-footer-group &#124; table-row &#124; table-column-group &#124; table-column &#124; table-cell &#124; table-caption &#124; flex &#124; inline-flex &#124; grid &#124; inline-grid &#124; list-item &#124; inline-list-item &#124; contents &#124; none | The display property specifies the type of box used for an HTML element (also check [Visibility](#visibility) and the runtime caveats appendix below). `-webkit-box`/`-webkit-inline-box` are also accepted under `STARFISH_ENABLE_CSS_WEBKIT_BOX_PREFIX`. **`flow-root`, `run-in`, `ruby*`, and multi-token L3 syntax (`block flex`) are silently dropped** — see the audit-additions table below. |  |
...
| | list-style-type | &lt;counter-style&gt; &#124; &lt;string&gt; &#124; none | Specifies the appearance of a list item element. **The CSS parser accepts any custom-identifier** (no validation against a known counter-style list); rendered marker support is limited to the keywords used by `<ul>`/`<ol type=...>`: `disc`, `decimal`, `lower-alpha`, `upper-alpha`, `lower-roman`, `upper-roman`, `disclosure-open`, `disclosure-closed`. Other CSS counter styles (`circle`, `square`, `decimal-leading-zero`, `armenian`, `georgian`, `cjk-ideographic`, `katakana`, …) parse without error but the marker glyph fall-back is undefined. String values (`list-style-type: "→ "`) parse and store but the renderer ignores the string in favor of a default bullet. | |
...
> **Footgun:** rejected `display` values produce **no console warning**. Authors that try `display: flow-root` and rely on `getComputedStyle.display` for feature detection will see the property fall back silently to its initial value (`inline`/`block`). Verify visually or by checking layout (e.g., `offsetWidth` on a child).
