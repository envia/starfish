...
try:
    ...
    from .telemetry_upload import upload_telemetry
except ImportError:  # script/installed-tools flat execution
    ...
    from progress.telemetry_upload import upload_telemetry
...
W3_FINALIZATION_STATE_FILE = "w3-finalization-state.json"
_W3_EXCEPTION_STOP_REASONS = {
    "round_limit",
    "repeated_failure",
    "no_net_progress",
    "validator_artifact_limit",
}
...
def cmd_workflow_start(args: argparse.Namespace) -> None:
    ...
    if workflow == "W3":
        # 이전 실행의 final record가 새 W3를 통과시키지 않게 세션별 증적을 초기화한다.
        _w3_finalization_state_path(output_dir).unlink(missing_ok=True)
    stats["workflows"][workflow]["start_time"] = datetime.now().isoformat(
        timespec="seconds"
    )
    stats["workflows"][workflow]["status"] = "in_progress"
    _save_runtime_stats(output_dir, stats)
...
def _w3_finalization_state_path(output_dir: Path) -> Path:
    return output_dir / W3_FINALIZATION_STATE_FILE


def cmd_record_w3_finalization(args: argparse.Namespace) -> None:
    """Persist one W3 finalize result so terminal status survives shell boundaries."""
    output_dir = Path(args.output_dir)
    path = _w3_finalization_state_path(output_dir)
    try:
    ...
    record = {
        ...
        "llm_repair_rounds": int(args.llm_repair_rounds),
        "failure_fingerprint": str(getattr(args, "failure_fingerprint", "") or ""),
    ...
    print(f"[ProgressTracker] W3 finalize result recorded: {path}")


def _derive_w3_terminal_status(output_dir: Path) -> str:
    path = _w3_finalization_state_path(output_dir)
    try:
        state = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise ValueError(f"W3 finalization state missing or invalid: {path}") from exc
    final = state.get("final") if isinstance(state, dict) else None
    if not isinstance(final, dict) or final.get("phase") != "final":
        raise ValueError("W3 finalization state has no final finalize record")

    exit_code = final.get("exit_code")
    if exit_code == 0:
        return "passed"
    rounds = final.get("llm_repair_rounds")
    reason = final.get("stop_reason")
    fingerprint = final.get("failure_fingerprint")
    issue = final.get("issue")
    records = state.get("records") if isinstance(state, dict) else None
    recorded_repair_rounds = {
        item.get("llm_repair_rounds")
        for item in records if isinstance(item, dict) and item.get("phase") == "repair"
    } if isinstance(records, list) else set()
    has_all_repair_records = (
        isinstance(rounds, int)
        and set(range(1, rounds + 1)).issubset(recorded_repair_rounds)
    )
    if (
        isinstance(rounds, int)
        and rounds >= 1
        and has_all_repair_records
        and isinstance(fingerprint, str)
        and fingerprint
        and reason in _W3_EXCEPTION_STOP_REASONS
        and (reason != "validator_artifact_limit" or (isinstance(issue, str) and issue))
    ):
        return "accepted-with-exceptions"
    raise ValueError(
        "W3 non-zero finalize cannot be accepted: record a final result after at least one "
        "recorded LLM repair rounds, a failure fingerprint, and an allowed stop reason "
        "(validator/artifact limits also require an issue)"
    )


def cmd_workflow_end(args: argparse.Namespace) -> None:
    """워크플로우 terminal 상태와 종료 시간을 기록한다."""
    output_dir = Path(args.output_dir)
    stats = _load_runtime_stats(output_dir)
    if not stats:
        print("[ERROR] No runtime-stats.json found. Run 'session-init' first.")
    ...
    token_usage = 0
    requested_status = str(getattr(args, "status", "") or "")
    if workflow == "W3":
        try:
            status = _derive_w3_terminal_status(output_dir)
        except ValueError as exc:
            print(f"[ERROR] {exc}", file=sys.stderr)
            raise SystemExit(2) from exc
        if requested_status and requested_status != status:
            print(
                f"[ERROR] Requested W3 status {requested_status!r} conflicts with "
                f"finalization evidence ({status!r})",
                file=sys.stderr,
            )
            raise SystemExit(2)
    else:
        status = requested_status or "passed"
    if status not in {"passed", "accepted-with-exceptions"}:
        print(f"[ERROR] Invalid terminal status: {status}")
        return
    ...
    stats["workflows"][workflow]["status"] = status
    ...
    print(f"[ProgressTracker] {workflow} terminal status: {status}")
...
def cmd_finalize_history(args: argparse.Namespace) -> None:
    ...
    # finalize(W3)·delta(W-DELTA) 종료 시 telemetry 전송 (best-effort, 실패해도 finalize 계속)
    upload_result = upload_telemetry(output_dir)
    upload_status = upload_result.get("status")
    if upload_status == "sent":
        print("[ProgressTracker] Telemetry 전송 완료 (code2spec/code2spec-telemetry)")
    elif upload_status == "error":
        # raise하지 않되 운영 디버깅을 위해 사유는 남긴다
        print(f"[ProgressTracker] Telemetry 전송 실패 (무시): {upload_result.get('reason')}")
...
def _create_session_history(
    ...
    for wf in _workflow_names_for_history(stats):
        ...
        terminal_status = wf_data.get("status")
        if terminal_status == "passed":
            status = "✅ passed"
        elif terminal_status == "accepted-with-exceptions":
            status = "⚠️ accepted-with-exceptions"
        elif wf_data.get("end_time"):
            # status 필드 도입 전 실행 이력도 기존의 완료 표시를 유지한다.
            status = "✅ passed (legacy)"
        elif terminal_status == "in_progress" or not wf_data.get("end_time"):
            status = "⏳"
        else:
