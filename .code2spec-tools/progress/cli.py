...
def main() -> None:
    ...
    p_we.add_argument(
        "--status",
        choices=["passed", "accepted-with-exceptions"],
        default="",
        help="W3에서는 finalization state와 일치할 때만 허용; 다른 workflow의 기본값은 passed",
    )

    # record-w3-finalization — shell/coding-agent 경계를 넘는 W3 finalize 증적
    p_w3f = sub.add_parser("record-w3-finalization", help="W3 finalize 결과를 영속 기록")
    p_w3f.add_argument("--output-dir", required=True)
    p_w3f.add_argument("--phase", required=True, choices=["initial", "repair", "final"])
    p_w3f.add_argument("--exit-code", required=True, type=int)
    p_w3f.add_argument("--llm-repair-rounds", required=True, type=int)
    p_w3f.add_argument("--failure-fingerprint", default="")
    p_w3f.add_argument("--issue", default="", help="validator/artifact 한계의 관련 이슈")
    p_w3f.add_argument(
        "--stop-reason",
        default="",
        choices=["", "round_limit", "repeated_failure", "no_net_progress", "validator_artifact_limit"],
    )
    ...
    elif args.command == "record-w3-finalization":
        cmd_record_w3_finalization(args)
