...
canonical 경로 (wiki-dir = ``<target>/code2spec``, analysis-dir = ``<wiki-dir>/.analysis``):
  ...
  module-deps      → ``.analysis/state/delta/module-deps.json``     (direct module-edge 산출)
...
MODULE_DEPS = WIKI_TOOLS / "module_deps.py"
SYNC_MODULE_DIAGRAM = WIKI_TOOLS / "sync_module_diagram.py"
...
class Paths:
    ...
    def __init__(self, repo: Path, output_dir: Path | None) -> None:
        ...
        self.module_deps = cp.module_deps                # state/delta/module-deps.json
...
def cmd_finalize(args: argparse.Namespace) -> None:
    ...
    # 2.5-a W2의 canonical module-groups와 실제 resolved import로부터 직접 모듈 edge를
    # 생성한다. 이 산출물은 W3가 module-dependency Mermaid를 만들고 검증할 때 공통 근거다.
    # W2 이전/legacy 산출물에 groups가 없으면 도구가 no-op으로 성공한다.
    run([py, MODULE_DEPS, "--wiki-dir", p.wiki_dir], cwd=p.repo, check=False)
    run([py, SYNC_MODULE_DIAGRAM, "--wiki-dir", p.wiki_dir], cwd=p.repo, check=False)
