...
Usage:
  ...
  python3 query_artifacts.py <wiki-dir> module-edges [--node ui_pages]
                                              [--from ui_pages] [--to ui_components]
...
MODES = ("summary", "list", "pages", "symbols", "edges", "module-edges")
...
def mode_module_edges(wiki_dir: str, args: dict[str, Any]) -> int:
    """module-deps.json 조회: W2 모듈 사이의 직접 import 근거만 돌려준다."""
    node = opt(args, "node")
    frm_filter = opt(args, "from")
    to_filter = opt(args, "to")
    path = os.path.join(wiki_dir, ".analysis", "state", "delta", "module-deps.json")
    if not os.path.isfile(path):
        print(f"error: not found: {path} (run wiki_cli.py finalize or module_deps.py first)",
              file=sys.stderr)
        return 1
    data = load_json(path)
    if not isinstance(data, dict) or data.get("schemaVersion") != 1:
        print(f"error: invalid module-deps artifact: {path}", file=sys.stderr)
        return 1
    rows: list[list[str]] = []
    for edge in data.get("edges") or []:
        if not isinstance(edge, dict):
            continue
        frm, to = str(edge.get("from") or ""), str(edge.get("to") or "")
        if node is not None and node not in frm and node not in to:
            continue
        if frm_filter is not None and frm_filter not in frm:
            continue
        ...
        sample = f"{first.get('from', '')} -> {first.get('to', '')}:{first.get('line', '')}"
        rows.append([frm, to, str(len(evidence)), sample])
    limit = opt_int(args, "limit", DEFAULT_LIMIT)
    offset = max(0, opt_int(args, "offset", 0))
    window = rows[offset:] if limit <= 0 else rows[offset:offset + limit]
    print("\t".join(["from", "to", "evidence_count", "sample_evidence"]))
    for row in window:
        print("\t".join(clip(cell, DEFAULT_WIDTH) for cell in row))
    footer("module-deps.json", len(rows), len(window), max(limit, 1), offset)
    return 0
...
def main() -> int:
    ...
    if mode == "module-edges":
        return mode_module_edges(wiki_dir, args)
