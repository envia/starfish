...
_DIAGRAM_TYPE_RE = re.compile(
    r"^\s*%%\s*code2spec:diagram-type=(module-dependency|system-context)\s*$", re.IGNORECASE | re.MULTILINE)
_SOURCE_LINK_RE = re.compile(r"\]\(src:([^\s)#]+)")
_EDGE_EVIDENCE_RE = re.compile(r"^\s*%%\s*code2spec:edge-evidence=([^|\s]+)\|src:([^#\s]+)#L([0-9]+)\s*$", re.I | re.M)
...
def parse_edges(block: str) -> list[dict[str, str]]:
    ...
    # A code2spec directive is a Mermaid comment and may precede the graph line.
    while parts and (not parts[0].strip() or parts[0].lstrip().startswith("%%")):
        parts.pop(0)
...
def diagram_type(block: str) -> str | None:
    """Return an explicitly declared validation model; legacy blocks stay strict."""
    match = _DIAGRAM_TYPE_RE.search(block)
    return match.group(1).lower() if match else None


def module_id(name: str) -> str:
    """Stable Mermaid-safe spelling for a module-group slug."""
    return name.strip().lower().replace("-", "_")


def module_owners(wiki_dir: str) -> dict[str, str]:
    """Read explicit W2 Relevant Source Files, rejecting ambiguous ownership.

    No directory-prefix or transitive ownership is inferred.  That keeps an LLM
    generated module label from becoming an escape hatch for unrelated files.
    """
    ...
    try:
        ...
        return {}
    for name in names:
        if not name.endswith(".md") or name.lower() in {"readme.md", "index.md"}:
            continue
        try:
            text = open(os.path.join(modules_dir, name), encoding="utf-8", errors="replace").read()
        except OSError:
            continue
        owner = module_id(posixpath.splitext(name)[0])
        for raw_path in _SOURCE_LINK_RE.findall(text):
            path = raw_path.replace("\\", "/").lstrip("/")
            candidates.setdefault(path, set()).add(owner)
    return {path: next(iter(owners)) for path, owners in candidates.items() if len(owners) == 1}


def canonical_module_owners(wiki_dir: str) -> dict[str, str] | None:
    """Read W2's canonical module ownership, returning None only when absent/unreadable.

    A present, valid groups file takes priority over prose Module Cards.  Empty and
    ambiguous groups are not filled by prefix inference; typed diagrams must then
    fail rather than gain accidental support.
    """
    path = os.path.join(wiki_dir, ".analysis", "state", "delta", "module-groups.yaml")
    if not os.path.isfile(path):
        return None
    try:
        raw = yaml.safe_load(open(path, encoding="utf-8", errors="replace"))
    ...
    for group in raw["modules"]:
        ...
        for item in group.get("files") or []:
            path_key = as_text(item).replace("\\", "/").lstrip("/")
            if path_key:
                candidates.setdefault(path_key, set()).add(module_id(group["name"]))
    return {path_key: next(iter(values)) for path_key, values in candidates.items()
            if len(values) == 1}
...
def load_module_edge_artifact(wiki_dir: str, deps_path: str | None,
                              owners: dict[str, str]) -> tuple[set[str], dict[str, list[dict[str, Any]]]] | None:
    """Load a current module-deps artifact, rejecting stale/malformed data."""
    artifact_path = os.path.join(wiki_dir, ".analysis", "state", "delta", "module-deps.json")
    groups_path = os.path.join(wiki_dir, ".analysis", "state", "delta", "module-groups.yaml")
    try:
        data = json.loads(open(artifact_path, encoding="utf-8").read())
    except (OSError, ValueError):
        return None
    input_ = data.get("input") if isinstance(data, dict) else None
    if not isinstance(input_, dict) or data.get("schemaVersion") != 1:
        return None
    if (not deps_path or input_.get("depsSha256") != _sha256_file(deps_path)
            or input_.get("moduleGroupsSha256") != _sha256_file(groups_path)):
        return None
    if set(data.get("modules") or []) != set(owners.values()):
        return None
    index: set[str] = set()
    ...
    for edge in data.get("edges") or []:
        if not isinstance(edge, dict):
            return None
        frm, to = as_text(edge.get("from")), as_text(edge.get("to"))
        if frm not in set(owners.values()) or to not in set(owners.values()):
            return None
        key = f"{frm}->{to}"
        items = edge.get("evidence")
        if not isinstance(items, list) or not items:
            return None
        index.add(key)
        evidence[key] = [x for x in items if isinstance(x, dict)]
    return index, evidence


def build_module_edge_index(deps: dict[str, Any], owners: dict[str, str]) -> tuple[set[str], dict[str, list[dict[str, Any]]]]:
    """Aggregate only direct, concretely resolved internal import edges by module."""
    index: set[str] = set()
    evidence: dict[str, list[dict[str, Any]]] = {}
    for imp in (deps.get("internal") or []):
        frm = as_text(imp.get("from"))
        to = as_text(imp.get("resolved_to"))
        source_module = owners.get(frm)
        target_module = owners.get(to)
        if not source_module or not target_module or source_module == target_module:
            continue
        key = f"{source_module}->{target_module}"
        index.add(key)
        evidence.setdefault(key, []).append({"from": frm, "to": to, "line": imp.get("line")})
    return index, evidence


def module_edge_supported(from_: str, to: str, owners: dict[str, str], edge_idx: set[str]) -> str:
    """Validate explicit module diagrams without fuzzy or transitive matching."""
    known_modules = set(owners.values())
    from_module = module_id(from_)
    to_module = module_id(to)
    if from_module not in known_modules or to_module not in known_modules:
        return "unknown-node"
    return "supported" if f"{from_module}->{to_module}" in edge_idx else "questionable"


def system_context_evidence(block: str) -> dict[str, tuple[str, int]]:
    """Explicit edge→source coordinates; no free-form semantic assertion is trusted."""
    return {m.group(1): (m.group(2), int(m.group(3))) for m in _EDGE_EVIDENCE_RE.finditer(block)}


def system_edge_supported(from_: str, to: str, ext: set[str], owners: dict[str, str],
                          evidence: dict[str, tuple[str, int]], repo_root: str | None) -> str:
    """Validate a system boundary via an explicit, in-range source evidence link.

    This deliberately accepts only module↔explicit-external edges.  It does not turn
    arbitrary graph labels or unmarked external systems into a pass.
    """
    internal = module_id(from_) if from_ not in ext else module_id(to)
    external = to if from_ not in ext else from_
    if from_ not in ext and to not in ext:
        return "unknown-node"
    if internal not in set(owners.values()) or external not in ext or not repo_root:
        return "unknown-node"
    coord = evidence.get(f"{from_}->{to}")
    if not coord:
        return "questionable"
    path, line = coord
    target = os.path.abspath(os.path.join(repo_root, path))
    if os.path.commonpath([os.path.abspath(repo_root), target]) != os.path.abspath(repo_root):
        return "questionable"
    try:
        return "supported" if 1 <= line <= len(open(target, encoding="utf-8", errors="replace").readlines()) else "questionable"
    except OSError:
        return "questionable"
...
def main() -> int:
    ...
    canonical_owners = canonical_module_owners(wiki_dir)
    owners = canonical_owners if canonical_owners is not None else module_owners(wiki_dir)
    artifact = load_module_edge_artifact(wiki_dir, deps_json, owners) if canonical_owners is not None else None
    if artifact is not None:
        module_edge_idx, module_evidence = artifact
        module_source = "module-deps"
    else:
        module_edge_idx, module_evidence = build_module_edge_index(deps, owners)
        module_source = "module-groups" if canonical_owners is not None else "module-cards"
    if owners:
        sources.append(f"{module_source} ({len(set(owners.values()))} modules, "
                       f"{len(module_edge_idx)} direct edges)")
    ...
    for fp in walk_markdown(wiki_dir):
        ...
        for blk in blocks:
            ...
            kind = diagram_type(blk)
            context_evidence = system_context_evidence(blk)
            for e in edges:
                evidence: list[dict[str, Any]] = []
                if kind == "system-context":
                    status = system_edge_supported(e["from"], e["to"], ext, owners,
                                                   context_evidence, repo_root)
                elif e["from"] in ext or e["to"] in ext:
                    status = "external"
                elif kind == "module-dependency":
                    status = module_edge_supported(e["from"], e["to"], owners, module_edge_idx)
                    evidence = module_evidence.get(
                        f"{module_id(e['from'])}->{module_id(e['to'])}", [])
                else:
                    # Legacy diagrams retain the pre-existing strict file/symbol
                    # validation model until explicitly migrated.
                    status = edge_supported(e["from"], e["to"], known, edge_idx)
                all_edges.append({"file": os.path.relpath(fp, wiki_dir), **e,
                                  "status": status, "diagram_type": kind or "legacy",
                                  "evidence": evidence})
