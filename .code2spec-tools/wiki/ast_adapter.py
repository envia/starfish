...
_RESOLVABLE_EXTENSIONS = (".ts", ".tsx", ".js", ".jsx", ".mjs", ".cjs", ".json")


def _jsonc_load(path: str) -> dict[str, Any]:
    """Read the small JSONC subset used by tsconfig without adding a dependency."""
    try:
        text = open(path, encoding="utf-8").read()
    except OSError:
        return {}
    # tsconfig permits comments and trailing commas.  This deliberately only
    # removes comment-shaped text outside quoted strings.
    out: list[str] = []
    quote = ""
    escaped = False
    i = 0
...
class ImportResolver:
    """Resolve only imports proven to name a file under the analysed repository.

    The raw import specifier remains in deps.json for compatibility.  Resolution is
    additive (`resolved_to`) and never guesses a package alias into an internal file.
    """

    def __init__(self, repo_root: str) -> None:
        self.root = os.path.abspath(repo_root)
        self.base_url = self.root
        self.paths: dict[str, list[str]] = {}
        for name in ("tsconfig.json", "jsconfig.json"):
            config = _jsonc_load(os.path.join(self.root, name))
            opts = config.get("compilerOptions") if isinstance(config, dict) else None
            if not isinstance(opts, dict):
                continue
            base = opts.get("baseUrl")
            if isinstance(base, str):
                self.base_url = os.path.abspath(os.path.join(self.root, base))
            raw_paths = opts.get("paths")
            if isinstance(raw_paths, dict):
                for key, values in raw_paths.items():
                    if isinstance(key, str) and isinstance(values, list):
                        self.paths[key] = [v for v in values if isinstance(v, str)]
            if self.paths:
                break

    def _existing_file(self, candidate: str) -> str | None:
        candidates = [candidate]
        if not os.path.splitext(candidate)[1]:
            candidates.extend(candidate + ext for ext in _RESOLVABLE_EXTENSIONS)
            candidates.extend(os.path.join(candidate, "index" + ext)
                              for ext in _RESOLVABLE_EXTENSIONS)
        for path in candidates:
            absolute = os.path.abspath(path)
            if os.path.isfile(absolute) and os.path.commonpath([self.root, absolute]) == self.root:
                return os.path.relpath(absolute, self.root).replace(os.sep, "/")
        return None

    def resolve(self, source_rel: str, specifier: Any) -> str | None:
        if not isinstance(specifier, str) or not specifier:
            return None
        if specifier.startswith("."):
            return self._existing_file(os.path.join(self.root, os.path.dirname(source_rel), specifier))
        for pattern, targets in self.paths.items():
            ...
            for target in targets:
                return_value = self._existing_file(os.path.join(self.base_url, target.replace("*", wildcard)))
                if return_value:
                    return return_value
        return None
...
def main() -> int:
    ...
    import_resolver = ImportResolver(REPO_ROOT)
    ...
    for e in edges:
        ...
        resolved_to = import_resolver.resolve(frm, to)
        if resolved_to:
            # Keep `to` as the source spelling; consumers that need a concrete
            # graph can rely on this provenance-backed path.
            entry["resolved_to"] = resolved_to
        internal = (isinstance(to, str) and (to.startswith(".") or to.startswith("/"))) \
            or resolved_to is not None
