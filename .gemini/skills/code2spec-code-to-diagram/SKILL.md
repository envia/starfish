...
- **Module dependency graphs require an explicit model marker.** When a graph uses W2 module
  slugs such as `ui_pages` or `lib_core` rather than AST file/symbol IDs, put
  `%% code2spec:diagram-type=module-dependency` inside the Mermaid block.
  Every edge must still be backed by a direct resolved file import between the two modules;
  never use the marker for transitive or conceptual relationships. Legacy graphs without the
  marker continue to use strict file/symbol validation.
  Before generating or editing one, query
  ``.analysis/state/delta/module-deps.json`` (or
  ``query_artifacts.py <OUTPUT_DIR> module-edges``): it is the canonical, direct-only
  file→module aggregation and includes the file/line evidence for every allowed edge.
