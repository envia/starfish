...
description: "W3(finalize)에서 검증 → verification-report.md 기록 → hard gate 교정 루프 → passed 또는 accepted-with-exceptions 종료 상태 기록을 수행하는 스킬. hard gate=requiredSlots·astBaseline·wikiStructure·mermaidSyntax·grounding·claimGrounding·diagramAccuracy·moduleFrPairing. soft=symbolCoverage·packageCoverage(측정, 게이트 아님). repo.json.qualityScore·qualityBreakdown과 .trust/claims.json을 읽어 요약·축별 근거·개선 내역을 기록한다. Use at the end of W3."
---
...
W3에서 **검증 → 리포트 기록 → hard gate 교정 루프 → 리포트 갱신**을 수행하고, 최종 상태를
`passed` 또는 `accepted-with-exceptions`로 명시한다.
...
| `<OUTPUT_DIR>/.analysis/state/delta/module-deps.json` | ⛔ | `… module-edges --from <Module> [--to <Module>]` |
...
> 단, 아래 유한 루프·인계 안전장치를 충족한 `accepted-with-exceptions`는 hard gate 통과가 아닌 명시적 인계 상태다.
...
1. **검증**: `wiki_cli.py finalize` 실행(→ `repo.json.qualityScore`·`.trust/*.json` 갱신).
   **이 루프는 W3를 주관하는 에이전트가 단독으로 수행한다 — `finalize`를 서브에이전트에게
   ...
   각 실행 직후 같은 셸 블록에서 종료 코드를 `${ANALYSIS_DIR}/w3-finalization-state.json`에 기록한다.
   셸 변수는 도구 호출 경계를 넘으면 사라질 수 있으므로 종료 코드·LLM 교정 횟수·실패 fingerprint를
   변수에만 보관하지 않는다. `finalize`가 non-zero여도 교정 루프를 계속해야 하므로, 최초 실행·각 repair
   재실행·최종 재검증 모두 `if finalize; then rc=0; else rc=$?; fi` 패턴으로 종료 코드를 캡처한다.
2. **기록**: Step 1~3으로 `verification-report.md`를 작성/갱신(현재 라운드 결과).
3. **판정**: hard gate 8축이 모두 통과(각 100 / N/A, claimGrounding은 contradiction 0)하면
   `passed`로 **루프 종료**한다.
4. **교정**: 통과 못 한 hard gate의 "틀린 부분"을 LLM이 직접 고친다(아래 축별 절차). 그 뒤
   **1번으로 돌아가 재검증**. 어떤 항목을 어떻게 고칠지는 각 축의 산출물이 위치(page:line)와 사유를
   ...
   - **`diagramAccuracy` 미지원 edge** — **삭제가 아니라 교정이 기본이다.** 다이어그램의 edge는 문서의
     ...
     ① Mermaid block에 `%% code2spec:diagram-type=module-dependency`가 있으면
        `query_artifacts.py … module-edges --from <A> [--to <B>]`로 W2 canonical module
        group의 **direct import** 근거를 확인한다. 출력에 없는 모듈 edge를 transitive/개념 관계로
        채우지 말고, 실제 출력된 edge로 수정하거나 marker를 제거하고 파일/심볼 모델로 다시 작성한다.
     ② 그 외 graph/flowchart는 `query_artifacts.py … edges --node <A> [--to <B>]`로 그 두 노드(또는 그 모듈의 파일들)가
...
> **유한 루프·인계 안전장치**: 교정 후 같은 실패 식별자(축·page·line·reason)가 반복되거나,
> hard gate 실패가 순감하지 않으면 LLM 교정은 최대 **5 라운드** 후 중단한다. 그 뒤 사실에 맞는 최종
> 교정(근거 없는 서술 삭제 또는 "코드에서 확인 불가" 표기)을 하고 **반드시 `finalize`를 한 번 더 실행**한다.
> 잔여 실패가 있으면 최종 상태를 `accepted-with-exceptions`로 기록한다. 이는 hard gate 통과가 아니며
> `finalize exit 0`을 주장해서는 안 되지만, 결과물 보존 및 다음 단계 인계는 허용한다.
>
> `verification-report.md`에는 잔여 항목의 좌표, 실패 원인, 수행한 교정, 품질 영향 및 후속 조치를
> 기록한다. 문서 교정으로 해결할 수 없는 validator/artifact 한계는 `blocked` 원인과 관련 이슈를 함께
> 기록한다. 점수를 위해 근거 없는 관계를 추가하거나 internal entity를 external로 위장하지 않는다.
>
> **상태 증적 규칙**: 최종 상태는 사람이 입력하지 않는다. 마지막 `finalize`가 exit 0이면 state의
> `final` record가 `passed`를 결정한다. non-zero이면 적어도 한 번의 LLM 교정과 허용된 중단 사유
> (`round_limit`·`repeated_failure`·`no_net_progress`·`validator_artifact_limit`)가 `final` record에
> 있어야 `accepted-with-exceptions`가 된다. 그렇지 않으면 W3는 미완성으로 남기며 인계하지 않는다.
>
...
- 헤더에 `verifiedAt`(최종 검증 시각), **총 라운드 수**, 최종 상태(`passed` / `accepted-with-exceptions`),
  그리고 hard gate 통과 여부를 적는다. `accepted-with-exceptions`는 통과가 아니다.
...
- `passed`: hard gate 8축이 최종적으로 모두 통과(100 / N/A, claimGrounding은 contradiction 0)하고
  최종 `w3-finalization-state.json` record의 `finalize exit 0`.
- `accepted-with-exceptions`: 5라운드 상한까지 교정을 실제 수행하고 최종 사실 교정·재검증을 마쳤지만
  hard gate 실패가 남은 상태. 잔여 항목을 2절에 전량 기록하고 통과로 표기하지 않는다.
- `blocked`는 별도의 성공 상태가 아니라 `accepted-with-exceptions`의 원인 분류다. validator/artifact
  한계와 관련 이슈를 리포트에 기록한다.
- 교정 루프를 수행하지 않은 채 실패 점수를 기록만 하고 종료를 선언하는 것은 허용되지 않는다.
